...
//
//        $this->assertFileDoesNotExist('custom-config-conf.php');
//
...
//    }
    ...
    public function testLoadingConfigFileDisallowArguments() {
        ...
        if(!file_exists('custom-config-conf.php')) {
            ...
            $customConfigFile = fopen("ehjwt-conf.php", "w");
            ...
            fwrite($customConfigFile, "return [\n");
            fwrite($customConfigFile, "'dsn' => 'mysql:host=localhost;dbname=ehjwt',\n");
            fwrite($customConfigFile, "'dbUser' => 'stdDonald',\n");
            fwrite($customConfigFile, "'dbPassword' => 'stcPassword',\n");
            fwrite($customConfigFile, "'jwtSecret' => 'stdSecret',\n");
            fwrite($customConfigFile, "'iss' => 'grifterinchief.com',\n");
            fwrite($customConfigFile, "'aud' => 'users',\n");
            fwrite($customConfigFile, "'disallowArguments' => 'true'\n");
            fwrite($customConfigFile, "];");
            ...
            $jwt = new EHJWT('not-secret', 'custom-config-conf.php', 'no-good-dsn', 'not-brad', 'not-password', 'not-site', 'not-user');
            $jwt->addOrUpdateIatProperty('10000');
            $jwt->addOrUpdateNbfProperty('0');
            $jwt->addOrUpdateSubProperty('1000');
            $jwt->addOrUpdateJtiProperty('1');
    ...
    public function testClearingCustomClaims() {
        ...
        $jwt->clearCustomClaim('location');

        $jwt->createToken();
        $claims = $jwt->getTokenClaims();

        $this->assertArrayNotHasKey('location', $claims);
    ...
    public function testMissingSignature() {
        ...
        $token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJ1c2VyIiwiaXNzIjoiQnJhZENoZXNuZXkuY29tIn0';
        $jwt = new EHJWT();
        $jwt->loadToken($token);
...
//
//        putenv('EHJWT_DSN=' . $dsn);
//
//        putenv('EHJWT_DB_USER=root');
//
//        putenv('EHJWT_DB_PASS=password');
//
//        putenv('EHJWT_JWT_SECRET=secrets');
//
//        putenv('EHJWT_ISS=rustbeltrebellion.com');
//
//        putenv('EHJWT_AUD=list255.com');
//
//        $now = time();
//        $expires = time() + 30 * 60;
//
...
//        putenv('EHJWT_AUD=list255.com');
//
//        putenv('EHJWT_USE_ENV_VARS=true');
//
//        $now = time();
//        $expires = time() + 30 * 60;
//
...
//        putenv('EHJWT_DSN');
//
//        putenv('EHJWT_DB_USER');
//
//        putenv('EHJWT_DB_PASS');
//
//        putenv('EHJWT_JWT_SECRET');
//
//        putenv('EHJWT_ISS');
//
//        putenv('EHJWT_AUD');
//
//        putenv('EHJWT_USE_ENV_VARS');
//    }
}
